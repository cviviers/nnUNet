...
from nnunetv2.run.load_spectre_pretrained_weights import load_spectre_pretrained_weights, download_from_huggingface
...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training:
        ...
        # first try final, then best, then latest
        expected_checkpoint_file = join(nnunet_trainer.output_folder, 'checkpoint_final.pth')
        if not isfile(expected_checkpoint_file):
            expected_checkpoint_file = join(nnunet_trainer.output_folder, 'checkpoint_best.pth')
            if not isfile(expected_checkpoint_file):
                expected_checkpoint_file = join(nnunet_trainer.output_folder, 'checkpoint_latest.pth')
                if not isfile(expected_checkpoint_file):
                    raise RuntimeError(f"Cannot run validation because there are no checkpoints available!")
        print(f'Loading checkpoint for validation: {expected_checkpoint_file}')
    else:
        if pretrained_weights_file is not None:
            # First try to load from user-specified path
            if not nnunet_trainer.was_initialized:
                nnunet_trainer.initialize()
            
            # Check if this is a SPECTRE trainer with a backbone
            if hasattr(nnunet_trainer.network, 'backbone'):
                try:
                    print(f"Attempting to load pretrained weights from: {pretrained_weights_file}")
                    load_spectre_pretrained_weights(nnunet_trainer.network.backbone, pretrained_weights_file, verbose=True)
                except Exception as e:
                    print(f"Failed to load from specified path: {e}")
                    print("Will attempt to download from HuggingFace instead...")
                    # If loading fails, try downloading from HuggingFace
                    try:
                        model_size = getattr(nnunet_trainer, 'model_size', 'L')
                        hf_weights_path = download_from_huggingface(model_size)
                        load_spectre_pretrained_weights(nnunet_trainer.network.backbone, hf_weights_path, verbose=True)
                        print(f"Successfully loaded pretrained weights from HuggingFace.")
                        print("Network: ", nnunet_trainer.network)
                    except Exception as hf_error:
                        print(f"Failed to download from HuggingFace: {hf_error}")
                        print("Training will continue without pretrained weights.")
            else:
                print(f"Warning: Trainer does not have a backbone attribute. Pretrained weights will not be loaded.")
        else:
            # No pretrained weights file specified - check if SPECTRE trainer and download from HuggingFace
            if not nnunet_trainer.was_initialized:
                nnunet_trainer.initialize()
            
            if hasattr(nnunet_trainer.network, 'backbone') and hasattr(nnunet_trainer, 'model_size'):
                try:
                    model_size = nnunet_trainer.model_size
                    print(f"No pretrained weights specified. Downloading {model_size} model from HuggingFace...")
                    hf_weights_path = download_from_huggingface(model_size)
                    load_spectre_pretrained_weights(nnunet_trainer.network.backbone, hf_weights_path, verbose=True)
                    print(f"Successfully loaded pretrained weights from HuggingFace.")
                    print("Network: ", nnunet_trainer.network)
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        if val_with_best:
            nnunet_trainer.load_checkpoint(join(nnunet_trainer.output_folder, 'checkpoint_best.pth'))
            print(f'Loaded checkpoint_dice_best for validation.')
...
def run_training_entry():
    ...
    parser.add_argument('-pretrained_weights', type=str, required=False, default=None,
                        help='[OPTIONAL] path to SPECTRE pretrained checkpoint file to be used as pretrained model. Will only '
                             'be used when actually training. If not specified, weights will be pulled from huggingface.')
