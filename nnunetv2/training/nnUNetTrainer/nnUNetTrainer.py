...
# from nnunetv2.utilities.get_spectre_from_plans import get_spectre_from_plans
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.device = device
        world_size = 1 if not self.is_ddp else dist.get_world_size()
        ...
        self.optimizer_class = torch.optim.AdamW # torch.optim.Adam, torch.optim.AdamW, torch.optim.SGD
        self.total_number_steps_per_epoch = self.num_iterations_per_epoch * world_size
        self.total_steps = self.num_epochs * self.total_number_steps_per_epoch
        self.add_decoder = True  # whether to add a decoder to the backbone (for spectre models)
        ...
        ### wandb init
        # add all to config for logging
        config = deepcopy(self.my_init_kwargs)
        wandb.init(entity='ginyu-force', project="nnUNet_spectre", config=config)

        self.print_to_log_file("\n#######################################################################\n"
                               "Please cite the following papers when using SPECTRE and nnU-Net:\n"
                               "Isensee, F., Jaeger, P. F., Kohl, S. A., Petersen, J., & Maier-Hein, K. H. (2021). "
                               "nnU-Net: a self-configuring method for deep learning-based biomedical image segmentation. "
                               "Nature methods, 18(2), 203-211.\n"
                               "Claessens, C., Viviers, C., D'Amicantonio, G., Bondarev, E., & van der Sommen, F. (2026). "
                               "Scaling Self-Supervised and Cross-Modal Pretraining for Volumetric CT Transformers. "
                               "arXiv preprint arXiv:2511.17209.\n"
                               "#######################################################################\n",
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        """
        # if "spectre" in architecture_class_name.lower():
            
        #     return get_spectre_from_plans(architecture_class_name=architecture_class_name, 
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        # check if mod has an attribute 'decoder'. If not, set deep supervision on the top level
    ...
    def on_train_epoch_start(self):
        ...
        self.print_to_log_file(
            f"Current learning rate: {self.optimizer.param_groups[0]['lr']:.6f} (initial: {self.initial_lr:.6f})")
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        wandb.log(loss_dict)
        wandb.log({'lr': self.optimizer.param_groups[0]['lr'], 'epoch': self.current_epoch})
        return loss_dict
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if not self.label_manager.has_regions:
            ...
            fp_hard = fp_hard[1:]
            fn_hard = fn_hard[1:]
        val_loss_dict = {'loss': l.detach().cpu().numpy(), 'tp_hard': tp_hard, 'fp_hard': fp_hard, 'fn_hard': fn_hard}
        wandb.log({'val_loss': val_loss_dict['loss'], 'tp_hard': np.sum(tp_hard), 'fp_hard': np.sum(fp_hard)})
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        wandb.log({'epoch_val_loss': loss_here, 'epoch_mean_fg_dice': mean_fg_dice, 'epoch': self.current_epoch})
    ...
    def on_epoch_end(self):
        ...
        # handle 'best' checkpointing. mean_fg_dice is computed by the logger and can be accessed like this
        if self._best_fgdice is None or self.logger.my_fantastic_logging['mean_fg_dice'][-1] > self._best_fgdice:
            self._best_fgdice = self.logger.my_fantastic_logging['mean_fg_dice'][-1]
            self.print_to_log_file(f"Yayy! New best Mean FG Dice: {np.round(self._best_fgdice, decimals=4)}")
            self.save_checkpoint(join(self.output_folder, 'checkpoint_dice_best.pth'))
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        self.network.eval()
        self.network.return_only_final_layer = True
